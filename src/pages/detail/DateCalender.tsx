...
export default class DateCalender extends Component<
  DateCalenderProps,
  DateCalenderState
> {
  ...
  render() {
    ...
    return (
      <RangePicker
        ...
        panelRender={(node) => {
          const date1 =
            (dates && dates[0] && dates[0].format('YYYY/MM/DD')) || '入住'
          const date2 =
            (dates && dates[1] && dates[1].format('YYYY/MM/DD')) || '退房'
          return (
            <div {...sharedProps}>
              <div className="p-4">
                <div className="flex items-center grid grid-cols-7 gap-4">
                  <div className="py-4 col-span-3" style={dayTotalStyle}>
                    <p className="font-bold text-xl">
                      {this.dateConverter(date1, date2)}晚
                    </p>
                    <p>
                      {date1} - {date2}
                    </p>
